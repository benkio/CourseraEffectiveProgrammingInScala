...
/** Interface for the heap data structure
  *
  * This interface defines the operations for constructing and manipulating a heap. A heap is modeled by the type `List[Node]`, and the only
  * way to construct a heap is to use the constructor `empty`, and then to use the operation `insert` to add more elements to it.
  *
  * @see
...
class Bogus2BinomialHeap extends BinomialHeap:
  ...
  override protected def link(node1: Node, node2: Node): Node = // node1.rank == node2.rank
    if node1.value <= node2.value then Node(node1.value, node1.rank + 1, node2 :: node2.children)
    else Node(node2.value, node2.rank + 1, node1 :: node2.children)
