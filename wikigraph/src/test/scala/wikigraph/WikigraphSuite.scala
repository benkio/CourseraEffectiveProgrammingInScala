...
import scala.util.{Success => TSucc}
import scala.util.{Failure => TFail}
...
class WikigraphSuite extends munit.FunSuite:
  ...
  property("zip with domain errors") {
    ...
    forAll(genAB) { (a: WikiResult[Int], b: WikiResult[String]) =>
      ...
      if errs.nonEmpty then (errs == obtained.swap.getOrElse(Seq.empty)) :| "Errors are not accumulated correctly"
      else (ar.toOption.flatMap(l => br.toOption.map(r => l -> r)) == obtained.toOption) :| "Result values are not zipped correctly"
    }
  }
  ...
  property("zip with domain errors and failures") {
    ...
    forAll(genAB) { (a: WikiResult[Int], b: WikiResult[String]) =>
      ...
      if ar.isFailure || br.isFailure then (List(ar, br).find(_.isFailure).get == obtained) :| "System failure is not reported correctly"
      else
        val errs: Seq[WikiError] = ar.get.swap.getOrElse(Seq.empty) ++ br.get.swap.getOrElse(Seq.empty)
        if errs.nonEmpty then (errs == obtained.get.swap.getOrElse(Seq.empty)) :| "Errors are not accumulated correctly"
        else
          (ar.get.toOption.flatMap(l => br.get.toOption.map(r => l -> r)) == obtained
            .get
            .toOption) :| "Results are not accumulated correctly"
    }
  }
    ...
    forAll(Gen.containerOf[List, WikiResult[Int]](wikiResultGen[Int](true, true))) { ls =>
      ...
      eits.collectFirst { case TFail(ex) => ex } match
        ...
        case None     =>
          ...
          if expErrors.nonEmpty then (expErrors.flatten == obtained.get.swap.getOrElse(Seq.empty)) :| "Errors are reported correctly"
          else (eits.collect { case TSucc(Right(value)) => value } == obtained.get.getOrElse(Seq.empty)) :| "Result sequence is correct"
    }
  }
  ...
  property("map") {
    forAll(wikiResultGen[Int](true, true)) { (w: WikiResult[Int]) =>
      ...
      Try(Await.result(w.value, timeout)) match
        ...
        case domainErr @ TSucc(Left(_)) =>
          (domainErr == obtained) :| "map on a WikiResult failed with a domain error is correct propagates the domain error"
  ...
  property("flatMap with a successful function") {
    forAll(wikiResultGen[Int](true, true)) { (w: WikiResult[Int]) =>
      ...
      Try(Await.result(w.value, timeout)) match
        case TSucc(Right(x))            =>
          (TSucc(Right((x + 2).toString)) == obtained) :| "flatMap on successfull WikiResult with a successful function is correct"
        case domainErr @ TSucc(Left(_)) =>
          (domainErr == obtained) :| "flatMap on a WikiResult failed with a domain error with a successful function propagates the error"
  ...
  property("flatMap with a function producing a domain error") {
    forAll(wikiResultGen[Int](true, true)) { (w: WikiResult[Int]) =>
      ...
      origin match
        ...
        case oldErr @ TSucc(Left(_)) =>
          (obtained == oldErr) :| "flatMap on a WikiResult with domain error using a failing function propagates the failure"
    }
  }
    ...
    forAll(wikiResultGen[Int](true, true)) { (w: WikiResult[Int]) =>
      ...
      origin match
        ...
        case TSucc(Right(_))         =>
          (obtained == TFail(
            ex
          )) :| "flatMap on a successfull WikiResult with a failing function produces a failed WikiResult with the correct system failure"
        case oldErr @ TSucc(Left(_)) =>
          (obtained == oldErr) :| "flatMap on a WikiResult with domain error using a failing function propagates the old error"
    }
  }
  ...
  test("distanceMatrix") {
    ...
    val res: Either[Seq[WikiError], Seq[(String, String, Option[Int])]] =
      Await.result(Wikigraph(InMemory(g)).distanceMatrix(List("TestArticle-1", "TestArticle-3")).value, timeout)
    val exp: Either[Seq[WikiError], Seq[(String, String, Option[Int])]] =
      Right(Seq(("TestArticle-1", "TestArticle-3", Some(2)), ("TestArticle-3", "TestArticle-1", Some(1))))
