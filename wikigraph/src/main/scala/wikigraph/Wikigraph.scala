...
final class Wikigraph(client: Wikipedia):
  ...
  /** Computes the distance between two articles using breadth first search.
    ...
    *
    * @return
    *   an asynchronous computation that might fail. If the maximal distance is exceeded during the search, the result is None
    *
    * Note: if a domain error occurs when jumping from node to node, fallback by ignoring the problematic node
    *
    * Hint: More information is provided in the description of the assignment Use the enqueue and dequeue methods of Queue.
    */
  def breadthFirstSearch(start: ArticleId, target: ArticleId, maxDepth: Int): WikiResult[Option[Int]] =
    ...
    /** This recursive method iterates on the graph.
      ...
      *   - When the queue is empty or the maxDepth is exceeded (in the next element of the queue), the search fails with None
      *   - Otherwise a node is retrieved from the queue and its neighbors fetched from the dataset. The search succeeds if `target` is in
      *     this set. Otherwise we recursively search after modifying `unknowns` and adding the unknown neighbors to the queue with the
      *     correct distance.
      *
      * @param visited
  ...
  /** Computes the distances between some pages provided the list of their titles. Do not compute the distance from page and itself.
    *
    * @param titles
