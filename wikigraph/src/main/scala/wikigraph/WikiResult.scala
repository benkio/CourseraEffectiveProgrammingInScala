...
/** The result of an asynchronous computation which may fail. This class is wrapper of Future[Either[Seq[WikiError], A]]
  *
  * @param A
  *   the type of the result of a successful computation
  * @param value
  *   the future to use to create this WikiResult. Checkout the companion object to find other ways to create a WikiResult
  *
...
case class WikiResult[A](val value: Future[Either[Seq[WikiError], A]]):
  ...
  /** Retrieve the results of two computations and produce a result containing the two results in a pair. If one of the computations fails,
    * its errors are propagated. If both fail, the errors of the two are propagated
    *
    * @param that
...
object WikiResult:
  ...
  /** Asynchronously and non-blockingly transforms a `Seq[A]` into a WikiResult[Seq[B]] using the provided `A => WikiResult[B]`.
    *
