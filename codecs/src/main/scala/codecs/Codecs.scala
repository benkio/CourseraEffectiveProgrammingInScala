...
trait ObjectEncoder[-A] extends Encoder[A]:
  ...
  /** Combines `this` encoder with `that` encoder. Returns an encoder producing a JSON object containing both fields of `this` encoder and
    * fields of `that` encoder.
    */
...
object ObjectEncoder:
  ...
  /** An encoder for values of type `A` that produces a JSON object with one field named according to the supplied `name` and containing the
    * encoded value.
    */
...
trait Decoder[+A]:
  ...
  /** Combines `this` decoder with `that` decoder. Returns a decoder that invokes both `this` decoder and `that` decoder and returns a pair
    * of decoded value in case both succeed, or `None` if at least one failed.
    */
...
trait DecoderInstances:
  ...
  /** A decoder for JSON arrays. It decodes each item of the array using the given `decoder`. The resulting decoder succeeds only if all the
    ...
    *
    * Hint: you should first check that the JSON value to decode is a JSON array, and if it is the case you should iterate over its JSON
    * elements (e.g., with `foldLeft`) to decode them. The value returned by `foldLeft` should be an `Option[List[A]]`. The first parameter
    * of `foldLeft` is the value to return in case the JSON array to decode is empty, so it would be a successful (`Some`) empty list of
    * `A`. Your iteration function should try to decode each element if the previous elements could be successfully decoded, otherwise it
    * should return `None`.
    */
  given listDecoder[A](
