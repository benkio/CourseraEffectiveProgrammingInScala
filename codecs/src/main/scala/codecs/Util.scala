...
object Util:
  ...
  /** Parse a JSON document contained in a `String` value into a `Json` value, returns `None` in case the supplied `s` value is not a valid
    * JSON document.
    */
  def parseJson(s: String): Option[Json] = Parser.parseFromString[Json](s).toOption

  /** Parse the JSON value from the supplied `s` parameter, and then try to decode it as a value of type `A` using the given `decoder`.
    *
