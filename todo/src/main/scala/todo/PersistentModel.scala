...
object PersistentModel extends Model:
  ...
  /** Load Tasks from a file. Return an empty task list if the file does not exist, and throws an exception if decoding the file fails.
    */
  def loadTasks(): Tasks =
    if Files.exists(tasksPath) then load[Tasks](tasksPath)
    else Tasks.empty

  /** Load an Id from a file. Returns Id(0) if the file does not exist, and throws an exception if decoding the file fails.
    */
  def loadId(): Id =
  ...
  /** Load JSON-encoded data from a file.
    ...
    *
    * It is not necessary to use this method. You should be able to use loadTasks and loadId instead, which have a simpler interface.
    */
  ...
  /** Save data to a file in JSON format.
    *
    * Given a file name and some data, saves that data to the file in JSON format. If the file already exists it is overwritten.
    *
    * It is not necessary to use this method. You should be able to use saveTasks and saveId instead, which have a simpler interface.
    */
  ...
  def create(task: Task): Id =
    ...
    val tks = tasks.toMap
    saveTasks(Tasks(tks + ((nextID, task))))
    saveId(nextID)
    nextID
  ...
  def delete(id: Id): Boolean =
    ...
    tks.size != newTasks.size

  def tasks: Tasks =
    loadTasks()

  def tasks(tag: Tag): Tasks =
    Tasks(tasks.tasks.filter(t => t._2.tags.contains(tag)))

  def complete(id: Id): Option[Task] =
    update(id)(_.complete)

  def tags: Tags =
    Tags(tasks.tasks.flatMap(_._2.tags).toList.distinct)
