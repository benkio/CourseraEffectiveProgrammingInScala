...
object Grade:
  ...
  def median(grades: Seq[Grade]): Grade =
    val gradesSorted = grades.sortBy(_.ordinal)
    if grades.size % 2 == 0 then gradesSorted(grades.size/2 - 1) else gradesSorted(grades.size/2)
end Grade
    ...
    if gradesPerCandidate.forall((candidate, grades) => grades.isEmpty) then
      ...
      val bestCandidates: Map[Candidate, Seq[Grade]] =
        gradesPerCandidate.filter {
          case (_, grades) => Grade.median(grades) == bestMedianGrade
        }
      ...
      if bestCandidates.size == 1 then
        ...
        val bestCandidatesMinusOneMedianGrade: Map[Candidate, Seq[Grade]] =
          bestCandidates.map {
            case (c, gs) => (c, gs.diff(Seq(bestMedianGrade)))
          }
